...
public class Hall implements Comparable<Hall>, Serializable {
	...
	public static Hall inputHall() {
		...
		System.out.print("Введіть назву кінозала: ");
		String name = scanner.nextLine();
		if (name.equals("")) {
			System.err.println("Назву кінозала введено некорректно!\n");
	...
	public boolean addTimeTableToHall() throws IllegalTimeFormatException {
		...
		if (isDone) {
			System.out.println("Графік роботи успішно добавлений в кінозал \"" + name + "\"!\n");
			return true;
		} else
			return false;
	}
	...
	public boolean removeTimeTableFromHall() {
		...
		if (isDone) {
			System.out.println("Графік роботи успішно видалений із кінозала \"" + name + "\"!\n");
			return true;
		} else
			return false;
	}
	...
	public boolean addScheduleToHall() throws IllegalTimeFormatException {
		...
		if (hallTimeTable.findDayInTimeTable(day).isPresent()) {
			...
			if (isScheduleTimeNonWorkable(day, hallTimeTable, schedule)) {
				...
				System.out.println("Розклад сеансів на " + day.toLiteral(true) + " успішно добавлено в кінозал \"" + name + "\"!\n");
				return true;
			} else {
				System.err.println("Добавлення розкладу сеансів на " + day.toLiteral(true) + " в кінозал \"" + name
						+ "\" неможливо,тому що існують сеанси в неробочий час кінозала!\n");
				return false;
			}
		} else {
			System.err.println(day.toLiteral(true) + " відсутній в графіку роботи!\n");
			return false;
		}
	}
	...
	public boolean removeScheduleFromHall() {
		...
		if (hallScheduleEntryFound.isPresent()) {
			...
			System.out.println("Розклад сеансів на " + day.toLiteral(true) + " успішно видалено із кинозала \"" + name + "\"!\n");
			return true;
		} else {
			System.err.println(day.toLiteral(true) + " відсутній в графіку роботи!\n");
			return false;
	...
	public void viewHallTimeTable() {
		System.out.print("Кінозал \"" + name + "\"\n");
		hallTimeTable.viewTimeTable();
		System.out.println();
	}
	
	public Function<Entry<Days, Schedule>, String> hallScheduleToString() {
		return entry -> "Розклад сеансів в " + entry.getKey().toLiteral(true) + ":\n" + entry.getValue().toString();
	}
	
	public void viewHallSchedule() {
		System.out.print("Розклад сеансів кінозала \"" + name + "\":\n");
