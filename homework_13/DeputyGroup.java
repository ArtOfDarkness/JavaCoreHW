...
public class DeputyGroup {
	...
	public void addDeputy() {
		...
		System.out.println("Чи бере депутат хабар (true/false):");
		...
		if (typeCorrect) {
			...
			while (iterator.hasNext()) {
				...
				if (nextDeputy.getSurname().equalsIgnoreCase(surname) && nextDeputy.getName().equalsIgnoreCase(name)) {
					iterator.remove();
					System.out.println("Депутат " + nextDeputy.toString() + " успішно виключений з фракції!");
				}
			}
	...
	public void getLargestBribeTaker() {
		...
		if (deputyIndex >= 0) {
			System.out.println("Найбільший хабар фракції " + deputyGroupArray.get(deputyIndex).toString());
		} else {
	...
	public void getAllDeputies() {
		System.out.println("Депутати які знаїодяться в даній фракції:");
		for (Deputy deputy : deputyGroupArray) {
			System.out.println(deputy.toString());
		}
	}
	...
	public void clearDeputyGroup() {
		...
		System.out.println("Всі депутати успішно видалені з фракції!");
	}
